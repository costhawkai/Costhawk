...
### costhawk_sync_claude_code

Sync Claude Code usage from local `~/.claude/projects` files. This parses your local session data and uploads usage metadata only.

**Parameters:**

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `dryRun` | boolean | No | Preview what would be sent without uploading |
| `startDate` | string | No | Only sync sessions after this date |
...
Sync OpenAI Codex CLI usage from local `~/.codex/sessions` files.
...
<Note>
  Codex sync can be disabled globally by setting `COSTHAWK_CODEX_AUTO_SYNC=false` in your environment.
</Note>
