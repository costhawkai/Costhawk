...
One command to set up CostHawk globally with auto-sync enabled:
...
<Tip>
  This sets your API key and enables auto-sync globally, so every new Claude Code session automatically syncs your usage data.
</Tip>
...
## Alternative: MCP-Only Setup

If you prefer not to set global env vars, you can pass them directly to the MCP server:

<CodeGroup>

```bash Global (All Projects)
claude mcp add -s user -e COSTHAWK_API_KEY=YOUR_TOKEN_HERE -e COSTHAWK_AUTO_SYNC=true costhawk -- npx -y costhawk@latest
```

```bash Current Project Only
claude mcp add -e COSTHAWK_API_KEY=YOUR_TOKEN_HERE -e COSTHAWK_AUTO_SYNC=true costhawk -- npx -y costhawk@latest
```
