...
|------|-------|--------------|
| Reporting Mode | MCP local sync and/or provider reporting APIs | Useful summaries, but limited enforcement and attribution |
| Control Mode (Recommended) | Wrapped keys + CostHawk proxy routing | Full per-request tracking, attribution, spending limits, and hard-stop enforcement |

<Warning>
  Read-only/reporting credentials are not enough for full proxy tracking. You need inference-capable provider keys to route live traffic through wrapped keys.
</Warning>
