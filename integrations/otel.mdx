...
Use a CostHawk ingest key in OTLP headers:

```bash
OTEL_EXPORTER_OTLP_HEADERS="x-costhawk-key=YOUR_INGEST_KEY"
OTEL_EXPORTER_OTLP_PROTOCOL=http/protobuf
```

The key should include the `otel:ingest` scope.

## Example Configuration

```bash
export OTEL_EXPORTER_OTLP_ENDPOINT=https://ingest.costhawk.ai
export OTEL_EXPORTER_OTLP_HEADERS="x-costhawk-key=ch_sk_..."
