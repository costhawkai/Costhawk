...
CostHawk offers two ways to track your AI API costs:

<CardGroup cols={2}>
  <Card title="MCP Server" icon="robot" href="#mcp-server-setup">
    For **Claude Code & Claude Desktop** users. Syncs your local usage data automatically — no code changes needed.
  </Card>
  <Card title="API Proxy" icon="shuffle" href="/integrations/overview">
    For **any application** calling OpenAI, Anthropic, or Google APIs. Change your base URL and every call is tracked with per-request cost data.
  </Card>
</CardGroup>
...
<Tabs>
  <Tab title="macOS/Linux">
    ...
    claude mcp add -s user -e COSTHAWK_API_KEY=YOUR_TOKEN_HERE -e COSTHAWK_AUTO_SYNC=true costhawk -- npx --yes costhawk@latest
    ```

    ```bash Current Project Only
    claude mcp add -e COSTHAWK_API_KEY=YOUR_TOKEN_HERE -e COSTHAWK_AUTO_SYNC=true costhawk -- npx --yes costhawk@latest
    ```
